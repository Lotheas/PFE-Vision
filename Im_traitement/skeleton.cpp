...
//	}
	...
	////affichage de mskel
    //qInfo() << "Mskel 1 :" << endlQt2;
	//matrixVisual(mskel[0]);

	////affichage de mskel
    //qInfo() << "Mskel 2 :" << endlQt2;
	...
	for (int i = 1; i < familySize / 2; i++)
	{
		...
		//Affichage des mskel
        /*qInfo() << "Mskel " << i * 2 + 1 << " :" << endlQt2;
		matrixVisual(mskel[i * 2]);
        qInfo() << "Mskel " << i * 2 + 2 << " :" << endlQt2;
		matrixVisual(mskel[i * 2 + 1]);*/
	}

    qInfo() << "\tExtraction du squelette..." << endlQt2;
	//int count = 0;
	bool done;
	do
	{
    ...
    qInfo() << "\tSquelette extrait. Temps ecoule :  " << ((double)clock() - (double)start) / (double)CLOCKS_PER_SEC << " secondes" << endlQt2;
		...
		for (int y = 1; y < image.cols - 1; y++)
		{
			...
			for (int xl = -1; xl <= 1; xl++)
			{
				for (int yl = -1; yl <= 1; yl++)
				{
					...
					{
                        qInfo() << endlQt2 << "element.at" << endlQt2;
					}*/
					/*try
					{*/
					...
					{
                        qInfo() << endlQt2 << "image.at" << endlQt2;
					}*/
...
void matrixVisual(Mat _m)
{
    ...
    qInfo() << QString::fromStdString(strOut) << endlQt2;
}

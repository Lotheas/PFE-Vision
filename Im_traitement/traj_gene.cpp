...
void trajectoryGeneration(Mat skeleton, bool ebranchage, std::vector<std::vector<Point>>* trajectories)
{
    ...
    qInfo() << "\tCalcul des trajectoires..." << endlQt3;
...
void trajectoryInRegion(Mat skeleton, vector<vector<Point>>* trajectories)
{
    ...
    for (uint i = 0; i < ptsClefs.size(); i++)
	{
		for (int xl = -1; xl <= 1; xl++)
		{
			for (int yl = -1; yl <= 1; yl++)
			{
				if (skeleton.at<uchar>(ptsClefs[i].pt.y + xl, ptsClefs[i].pt.x + yl))
				{
                    ...
                    for (uint j = 0; j < ptsClefs.size(); j++)
					{
	...
	do
	{
		...
		if (index < ptsClefs.size())
		{
            ...
            //qInfo() << "New trajectory (no " << trajectories->size() << " )" << endlQt3;
